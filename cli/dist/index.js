...
var CONFIG_FILE = "fonasa-ui.json";
function loadConfig() {
  const configPath = resolve2(process.cwd(), CONFIG_FILE);
...
function resolveInternalDeps(requested, allComponents) {
  const resolved = /* @__PURE__ */ new Map();
  function resolve6(entry) {
    ...
    if (entry.internalDeps && entry.internalDeps.length > 0) {
      for (const depName of entry.internalDeps) {
        const dep = allComponents.find(
          (c) => c.file.toLowerCase() === depName.toLowerCase() + ".tsx" || c.name.toLowerCase() === depName.toLowerCase()
        );
        if (dep) {
          resolve6(dep);
        }
...
async function fetchComponentSource(fileName) {
  ...
  const candidates = [
    ...
    resolve3(__dirname, "components", fileName),
    // 2. Desde la raíz del repo (cli/dist/ → ../../src/componentsUI/)
    resolve3(__dirname, "..", "..", "src", "componentsUI", fileName),
    // 3. Modo desarrollo (cwd es la raíz del repo)
    resolve3(process.cwd(), "src", "componentsUI", fileName)
  ];
  for (const candidate of candidates) {
...
var addCommand = new Command("add").description("Agrega uno o m\xE1s componentes a tu proyecto").argument("<componentes...>", "Nombres de los componentes a instalar").option("-y, --yes", "Instalar sin confirmaci\xF3n", false).option("-o, --overwrite", "Sobrescribir archivos existentes", false).option("-t, --with-tests", "Incluir archivos de test (Jest)", false).option("--only-tests", "Instalar solo los tests (sin copiar componentes)", false).option("--contacto", "Solo iconos de contacto (mesa telef\xF3nica, consultas)", false).option("--fonasa", "Solo iconos de Fonasa (logos institucionales)", false).option("--gob-chile", "Solo iconos de Gobierno de Chile", false).option("--clave-unica", "Solo icono de Clave\xDAnica", false).option("--rrss", "Solo iconos de redes sociales", false).action(async (componentes, opts) => {
  ...
  for (const name of componentes) {
    const entry = registry.find(
      (r) => r.name.toLowerCase() === name.toLowerCase() || r.file.toLowerCase() === name.toLowerCase() + ".tsx"
    );
    if (!entry) {
      notFound.push(name);
    } else {
...
var initCommand = new Command("init").description("Inicializa la configuraci\xF3n de fonasa-ui en tu proyecto").action(async () => {
  ...
  if (existsSync5(configPath)) {
    ...
    const { overwrite } = await (0, import_prompts.default)({
      type: "confirm",
      name: "overwrite",
      message: "\xBFDeseas sobrescribirlo?",
      initial: false
    });
    if (!overwrite) {
  ...
  const { componentsDir, typescript, includeTests } = await (0, import_prompts.default)([
    {
      type: "text",
      name: "componentsDir",
      message: "\xBFD\xF3nde quieres guardar los componentes?",
