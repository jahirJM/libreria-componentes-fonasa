...
function VariantCodeModal({
  ...
  function formatCode(code: string): string {
    // Si ya tiene saltos de línea, devolver tal cual
    if (code.includes("\n")) return code;
    // Match: <Component prop1={} prop2={} ... />  o  <Component prop1={} ...>
    const match = code.match(/^(<\w+)\s+(.*?)\s*(\/?>)(.*)$/s);
    if (!match) return code;
    const [, tag, propsStr, closing, rest] = match;
    // Separar props respetando llaves y strings
    const props: string[] = [];
    let current = "";
    let depth = 0;
  ...
  return (
    <>
      <div className="rounded-xl border border-gray-800 bg-gray-900/50 overflow-hidden flex flex-col h-full">
        {/* Preview */}
        <div className={`bg-white p-4 flex items-center justify-center flex-1 ${variant.responsive ? "min-h-[440px]" : "h-[200px]"} overflow-hidden`}>
          <div className={variant.responsive ? "w-full h-full relative" : "w-full max-w-xs"}>
            {variant.render()}
          </div>
        </div>
...
export function ComponentPreview({ entry }: ComponentPreviewProps) {
  ...
  return (
    <section className="flex gap-0 overflow-hidden">
      ...
      <div
        className={`sticky top-20 self-stretch transition-all duration-300 overflow-hidden shrink-0 ${
